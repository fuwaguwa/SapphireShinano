...
export class RealbooruAutocompleteHandler extends InteractionHandler
{
	...
	{
		if (interaction.commandName !== "booru" || interaction.options.getSubcommand() !== "realbooru") return this.none();
		...
		switch (focusedOption.name)
		{
			case "tags": {
				...
				const response = await fetch(`https://realbooru.com/index.php?page=dapi&s=tag&q=index&name_pattern=${searchArray[searchArray.length - 1]}%`);
